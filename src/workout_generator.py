...
def _load_workout(workout_name):
...
def load_workouts():
    with open(WORKOUTS_FILENAME, 'r') as workouts_file:
        ...
        _resolve_inheritance(workouts, workouts)
        workouts = _filter_partial_workouts(workouts)
    return workouts


def _filter_partial_workouts(workouts):
...
def _select_exercises(workout, exercise_dataframe):
    dfs = []
...
def _convert_to_template(exercises):
    template = []
...
def resample_exercise(exercise):
    ...
    exercise_dataframe = _apply_filters(exercise_dataframe)
    ...
    return _convert_to_template(exercises)
...
def generate_workout(workout_name, week):
    ...
    workout = _load_workout(full_workout_name)
    exercises = _select_exercises(workout, exercise_dataframe)
    _save_exercises(exercises)
    return _convert_to_template(exercises)
